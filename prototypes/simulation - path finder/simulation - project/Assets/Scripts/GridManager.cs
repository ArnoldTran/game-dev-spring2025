...
public class GridManager : MonoBehaviour
{
    ...
    [SerializeField]
    TMP_Text cellInfoText;

    [SerializeField]
    TMP_Text DayCounterText;

    Coroutine pathfindingCoroutine;
    float pathfindingSpeed = 0.01f;
    ...
    void Update()
    {
        ...
        indeceseText.text = "Road Cost:" + roadCost; //gridPosition.ToString();
        ...
        if (Physics.Raycast(ray, out hit, float.MaxValue, LayerMask.GetMask("cell")))
        {
            ...
            // Update UI with current position
            cellInfoText.text = "Cell: [" + cs.State.x.ToString() + "," + cs.State.y.ToString() + "]"; //gridPosition.ToString();

           

            // Display the type of the current cell
            if (cs.State.cellTerrain == CellState.terainType.GRASS)
            {
                cellInfoText.text += "\nType: Grass";
            }
            if (cs.State.cellTerrain == CellState.terainType.HOUSE)
            {
                cellInfoText.text += "\nType: House";
            }
            if (cs.State.cellTerrain == CellState.terainType.ROAD)
            {
                cellInfoText.text += "\nType: Road";
            }
            if (cs.State.cellTerrain == CellState.terainType.DIRT)
            {
                cellInfoText.text += "\nType: Dirt";
            }
            ...
            List<CellScript> neighbors = GetNeighbors(cs);

            // Append the neighboring cells' terrain types to the text
            string neighborTerrainTypes = "Neighboring Cells = ";
            foreach (var neighbor in neighbors)
            {
                // Append terrain type of each neighbor to the string
                neighborTerrainTypes += neighbor.State.cellTerrain.ToString() + ", ";
            }

            // Remove the last comma and space for cleaner output
            if (neighborTerrainTypes.EndsWith(", "))
            {
                neighborTerrainTypes = neighborTerrainTypes.Substring(0, neighborTerrainTypes.Length - 2);
            }

            // Append the neighbor terrain types to the cell info text
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (startCell == null)
                {
                    ...
                    {
                        if (cell.State.pathStateVisuals != "path")
                        {
        ...
        while (openSet.Count > 0) {
            ...
            if (current == endCell) {
                ...
                foreach (CellScript cell in path) {
                    ...
                    {
                        cell.State.cellTerrain = CellState.terainType.ROAD;
            ...
            if (current != startCell && current != endCell) {
                if (current.State.pathStateVisuals != "path")
                {
            ...
            foreach (var neighbor in GetNeighbors(current)) {
                ...
                if (!openSet.Contains(neighbor)) {
                    ...
                    if (neighbor.State.pathStateVisuals != "path")
                    {
            ...
            Debug.Log(CalculateCost(startCell, endCell));
            yield return new WaitForSeconds(pathfindingSpeed);
    ...
    public void GenerateGrid() {
        ...
        for (int x = 0; x < gridW; x++) {
            for (int y = 0; y < gridH; y++) {
                ...
                if (choice == 0)
                {
                    cs.State.cellTerrain = CellState.terainType.DIRT;
                }
                else
                {
                    cs.State.cellTerrain = CellState.terainType.GRASS;
                }
